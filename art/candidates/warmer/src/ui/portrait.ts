...
const SKIN = ['#ffe0c4', '#f8cfa2', '#e8ab72', '#cf8c50', '#a35f2f', '#7a441f'];
...
const EYES = ['#2f7fbe', '#4a8a3e', '#7a5220', '#95436a', '#3b3b4c', '#2f8080', '#c04452', '#7d54c0', '#d89a26'];
...
function brows(fill: Fill, p: Palette, b: Build): void {
  ...
  const left = b.x + Math.round((b.w - span) / 2);
  const right = left + b.eyeW + b.gap;
  const y = b.eyeY - (p.mood === 'stern' ? 5 : p.mood === 'bright' ? 7 : 6);
