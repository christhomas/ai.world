...
import { columns, dome, rows, brightness, shade, wash, type Fill } from './pixels';
...
/**
 * The light a face is seen in. Winter sun is thin and comes off the sky rather than out of it, so
 * it pales skin and hair and drains the warmth from both at once. Mixed into the palettes here
 * rather than laid over the finished drawing, so that a shadow of pale skin is pale too, and so
 * that one number moves the whole village at once.
 */
const WINTER_LIGHT = '#f2f0f8';
const WINTER_WASH = 0.24;

const SKIN = ['#ffdfc0', '#f4cba4', '#e0ab7d', '#c08a5c', '#96603a', '#6d4326']
  .map((colour) => wash(colour, WINTER_LIGHT, WINTER_WASH));
 ...
 * The wash closes every gap by the same fraction, so the bar comes down with it: otherwise winter
 * would quietly take half the hair colours away from anyone with a pale face.
 */
const CONTRAST = 0.17 * (1 - WINTER_WASH);
