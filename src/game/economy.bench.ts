 ...
 *
 * `DAYS` was 100 and both this file's benches read it. A hundred days is barely more than one
 * lifetime — people live 60 to 90 — so a world breeding under replacement has not had time to show
 * what that does to it, and every question of the form *does this village recover, or does it
 * flatten out* stopped before its answer arrived. Item #248 is waiting on exactly that, and no
 * constant in the economy should move until it can be tried rather than argued about.
 *
 * Through the environment rather than an argument, and the reason is `RUNS`: the runs are a module
 * constant that every bench reads, built once when the module loads, and threading a parameter up
 * through that would mean every caller passing a length it does not care about. `chore economy 400`
 ...
 *
 * In this file rather than a module of its own, and that is the reachability bench's doing rather
 * than taste. It counts an export reached only from a `.bench.ts` as reached by nobody, so a
 * `benchdays.ts` beside this one put a live function on the orphan list and the ratchet went up by
 * one for work that is called every time `chore economy` runs. One consumer, so one file.
 */
...
/** How long a village is lived for. A hundred days, or whatever `BENCH_DAYS` asked for. */
