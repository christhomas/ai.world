...
export function generateWebGraph(seed: number, radius = EDGE_OF_THE_WORLD): RoadGraph & { mesh: WorldMesh } {
   ...
   *
   * No islands in what `sectorMix` is shown, because there are none yet — and none is wanted. An
  ...
  const stands = standingOf({ islands: [], sectors, sectorOffset } as unknown as RoadGraph, levelNoise);
