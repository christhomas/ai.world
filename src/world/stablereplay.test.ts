...
/*
 * Seed 1234 at two hundred days, which is a village whose rails are full and whose farmers can
 * afford to widen them — `whichFarmerBuilds` refuses on both counts and most villages fail one.
 * Found by scanning rather than chosen: on seed 7 nothing is ever bought and every assertion below
 * would pass without measuring anything, which is its own kind of failure.
 */
const aVillage = (): Register => {
  const book = new Register(1234, 1);
