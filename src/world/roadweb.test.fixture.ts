...
import {
  FaceKind, MESH, faceAt, isLand,
  type MeshFace, type MeshVertex, type WorldMesh,
} from './mesh';
 ...
 * hollows hold a lake.
 *
 * Moved here from `mesh.ts` when the bounded country it grew retired with #192: `generateWebGraph`
 * below is still a real caller, and `mesh.test.ts` / `ranges.test.ts` keep the rest of this file's
