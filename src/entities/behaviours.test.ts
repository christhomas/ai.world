...
describe('the behaviour files', () => {
  ...
  it('send a fisherman down to the water and keep him there all day', () => {
    /*
     * A sailor's hours with a woodcutter's middle, which is what the trade is: a sailor goes away and a
  ...
  it('send a woodcutter out to the trees and keep him there all day', () => {
    /*
     ...
     */
    const day = JSON.stringify(villagers.woodcutter);
    expect(day).toContain('"woods"');
    expect(day, 'a woodcutter who walks to the trees and does not cut them').toContain('"dig"');
