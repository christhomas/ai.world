...
- [ ] **72. A Flutter client for iOS and Android, built to the Ledger II design.** Asked for
      ...
      **Decision — a native 3D surface, not Flutter's 2D canvas.** The app owns its Ledger II
      interface in Flutter widgets, while an opaque `Texture` supplies the world. The native
      renderer owns the depth buffer, terrain meshes, skeletal rigs, PCF shadows, water and season
      shaders, and the camera-relative cutaway. That preserves the WebGL build's low-poly geometry
      and shader semantics without asking `CustomPainter` to become a second 3D engine. The Dart
      seam is deliberately narrow: decoded chunk parcels, snapshots, camera and input state, and a
      texture identifier; no platform graphics object reaches the widget tree.

      This chooses the architecture; it does not pretend the renderer has already proved itself.
      The first Android and iOS spike must render one streamed terrain chunk, one creature rig and
      the cutaway volume through that texture on mid-range physical hardware, measuring frame time,
      memory and lifecycle recovery. Only then does the app take on the rest of the chunk mesher and
      protocol client. Failure of that spike reopens this decision with evidence rather than leaving
      every downstream interface waiting for an argument.
...
## A price is a reading, not a constant — September 16th

Three items out of one evening's reading of the economy, in dependency order: 139 needs 138's
price, 140 needs 139's shelf. Carriers between settlements follow as a fourth when these stand.

- [ ] **138. Every price in this world is a constant, so one village is the same shop as the next.**
      `deeds.ts` has taken a `price` argument since it was written, and every caller hands it a
      literal — `items.ts` for the hero's shelf, `farmbuilds.ts` for a stable, `works.ts` for a
      commission. A village with three farmers and no hunter sells meat at exactly the price of a
      village with three hunters and no field. Nothing a place actually *is* reaches what it
      charges, which means the map has no economic shape: there is no reason to walk to the next
      valley to buy anything, and no reason a seller in one place does better than a seller in
      another beyond how many neighbours he has.

      **What makes this cheap is that the stock is already there and already audited.** `food.ts`
      keeps a cellar and its cap, `fields.ts` answers `foodAt`, `harvest.ts` runs the cattle and the
      fishing, `holdings.ts` knows who works what and `livelihoods.ts` already credits a seller "the
      true market value of what they sold". Not one new thing has to be measured. What changes is
      that a price stops being declared and starts being read — from the store the village is
      audited on every morning anyway.

      The hero reads the same price, because `tills.ts` already lands his money in the villagers'
      purses and a player paying by a different rule from the people around him would be two
      representations of one act — the fault this project keeps finding at every scale.

      Open, and for the design to settle: the shape of the curve between plenty and scarcity, its
      floor and its ceiling, and whether a price belongs to a village or to a shop.

- [ ] **139. A shop's shelf cannot run out, so nothing the hero buys costs the village anything.**
      A `ShopDef` in `shops.ts` is a list of item ids in the order they are offered, and that is the
      whole of it — no counts, no depletion, no recovery. Buy every loaf in a hamlet and the hamlet
      has exactly as many loaves as it started with.

      The shelf becomes the village's own store: buying takes from it, selling adds to it. That is
      worth more than the honesty of it. With 138 in, the hero moving a price *is* the hero moving
      the goods, so hauling produce between valleys pays and a route can be worked out — and it
      limits itself, because the village he keeps buying from runs short and dear while the one he
      keeps selling to grows full and cheap. No rule anywhere has to say "you may not do this too
      much". `SELL_SHARE` already discounts what a shop pays for what you bring it, so a spread
      exists to build on.

      **The replay is the real work here, and it is a known shape rather than a new one.** A price
      that depends on what one player bought this afternoon cannot be derived from a seed, so a
      purchase has to be told, on the day it happened, and reach `relive` — exactly the seam items
      99 and 100 answered for a commissioned stable and for a village raising its own roof. Stamping
      a replayed purchase with today's date is the failure mode, as it was in 88.

- [ ] **140. The smithy and the apothecary are standing in villages with nobody in them.**
      `structures.ts` has had `ShopType = 'store' | 'smith' | 'inn' | 'apothecary'` for as long as
      villages have had shops, and it places them. But `TRADES` holds fourteen trades and neither
      smith nor apothecary is one of them — which `prosperity.ts` already records, from the other
      side, as the reason its higher wage once reached nobody. So the game sells swords that nobody
      in the world makes, out of buildings that nobody works in.

      Most of the machinery is in place. A trade already declares what it `needs` — a seller needs a
      market, an innkeeper an inn — so a smith needing a smithy is a sentence of the same shape as
      the ones already there. `vacancies.ts` staffs a new trade on its own out of `weight`, with
      nothing told to hire anybody. The inputs already have producers: a miner brings up ore and a
      woodcutter fells timber, and `farmbuilds.ts` has already established that a build can be
      priced in material out of a yard rather than in gold.

      What is genuinely missing is one idea: **a good made out of other goods.** Ore and timber in,
      gear onto the shelf 139 gave it, priced by 138's rule rather than by a special case for
      things the player buys.

      Open: whether making is a daily rate, like a farmer's day, or a commission with a lead time,
      which `works.ts` already knows how to run.

Also raised the same evening and deliberately not numbered yet, so that choosing one of them is a
decision somebody makes rather than a thing that drifts: carriers moving goods and money between
settlements along the road tree (the natural 141, and the only answer to the mines being the single
source of new money in the world); debt, which `purses.ts` floors at zero on purpose and `works.ts`
already half-runs as a deposit and a balance; what a village builds once it is past a bath house;
marriage and households, which nothing in `Person` expresses; heritable features, which
`movingon.ts` names as the thing its own existence was built to unlock; skill and mastery, so that a
farmer of forty days is not a farmer of one; old age; and illness, so that a doctor matters before
somebody is wounded. A graphics pass was also worked through — tone mapping first, then baked
vertex ambient occlusion, slope-tinted cliffs, sky-matched fog, coast foam and chamfered edges, in
that order and for that reason — and is parked at the same point: discussed, agreed in order, not
yet numbered.

**Found while reading, and worth fixing whenever somebody is next in that file:**
`src/game/economy.test.ts:807` prints `A purse goes into the ground with its owner: there is no
inheritance anywhere in this world.` as a hardcoded note. It has been false since `inheritance.ts`
landed on the 10th, and `prosperity.ts` already reasons from the opposite fact. The report's note
about fourteen trades sharing one wage is stale in the same way, and for the same reason: a bench
that states a conclusion it no longer measures will mislead the next person to read it, which it
did.
